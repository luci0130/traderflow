{{--
    Overridden from filament/filament to allow a navigation group to keep an
    icon while its items also keep their own icons. Filament's default view
    throws an exception in that case; here we render both so the sidebar shows
    a category icon on the group header and an icon on every link beneath it.

    The only change from the vendor view is the `if ($icon)` block further down,
    which no longer nulls the item icons (or throws) outside of the collapsed
    sidebar dropdown.
--}}
@props([
    'active' => false,
    ...
    >
        @foreach ($items as $item)
            @php
                ...
                // Unlike the vendor view, we keep the item icons even when the
                // group has its own icon. Only the collapsed-sidebar dropdown
                // ($hasDropdown) hides them, because it renders its own list.
                if ($icon && $hasDropdown) {
                    $itemIcon = null;
                    $itemActiveIcon = null;
                }
